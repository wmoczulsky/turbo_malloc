...
// actually only last bits of ptr are interesting
#ifndef NDEBUG
    #define CANARY_START ;uint8_t canary_start ;
    #define CANARY_END ;uint8_t canary_end ;
    #define INIT_CANARY(THIS, NAME)  THIS->canary_start = (size_t)#NAME "_start" ; THIS->canary_end = (size_t)#NAME "_end" ;
    #define CHECK_CANARY(THIS, NAME) assert(THIS->canary_start == (uint8_t)(size_t)#NAME "_start") ; assert(THIS->canary_end == (uint8_t)(size_t)#NAME "_end") ;
#else
    #define CANARY_START ;
    #define CANARY_END ;
